...
export const ProjectCard: React.FC<ProjectCardProps> = ({ item }) => {
  ...
  const hasLiveDemo = Boolean(item.demoUrl && item.demoUrl.trim().length > 0);
      ...
      <div className="px-8 mb-8 flex-grow">
        <div className="flex flex-wrap gap-1.5">
          {item.tags.map((tag) => (
            <span
              key={tag}
              className="px-2 py-0.5 border border-black/5 text-[10px] mono uppercase text-gray-400 font-bold group-hover:text-indigo-600 group-hover:border-indigo-600/20 transition-all"
            >
              {tag}
            </span>
          ))}
        ...
        <div className="grid grid-cols-1 gap-2">
          {isTemplate ? (
            <div className="grid grid-cols-2 gap-2">
              {/* Demo button: if demoUrl exists, open live demo in new tab; otherwise go to internal detail page */}
              {hasLiveDemo ? (
                <a
                  href={item.demoUrl}
                  target="_blank"
              ...
              {/* Optional: internal details link if you want it visible even when a live demo exists */}
              {/* <Link
                to={detailUrl}
                className="col-span-2 flex items-center justify-center px-4 py-3 bg-white text-black text-[11px] font-bold uppercase tracking-widest border border-black/5 hover:border-indigo-600 hover:text-indigo-600 transition-all"
              >
